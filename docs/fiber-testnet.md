...
2. 建立一个500ckb的channel: nodeA (500ckb) ⟺ node1 (250ckb)

   _node1配置的open_channel_auto_accept_min_ckb_funding_amount是438ckb，所以请传入500ckb及以上的funding_amount。_
...
3. 查询nodeA和node1之间channels
   ...
   **注意：channel刚变为CHANNEL_READY状态时，向其发送send_payment，仍可能报`error: Failed to build route`，可以等待一段时间后再重试。**
	...
	**为什么 local_balance 是 0xa32aef600 (43800000000)，remote_balance 是 0x460913c00 (18800000000)？**
	...
	由于每个 cell 至少需要 62 ckb，因此会预留这部分资金，以确保在链上结算（channel关闭）时有足够的 ckb 支付 cell 占用费用。在链上结算时，这 62 ckb 会被返还给各自的节点。
...
4. 调用node2的`new_invoice`接口生成一个invoice
   ...
   amount设置成 0x5f5e100 (100000000 shannon)，也就是1个ckb。payment_preimage需要使用唯一的32字节16进制数。
...
5. 让nodeA付款前先查询一下各channel的local_balance和remote_balance
   ...
   ```
    ...
    ```
   找出response中`funding_udt_type_script`为null的数据
   ```json
...
8. 再次查询各channel的local_balance和remote_balance
   ...
   ​	43499700000 - 43800000000 = -300300000 

   ​	4993500000000 + 19100300000 - 4993800000000 - 18800000000 = 300000  
   
   ​	5019100000000 - 5018800000000 =300000000  
...
2. 建立一个20RUSD的channel: nodeA (20RUSD) ⟺ node1 (0)

   _node1配置的RUSD的auto_accept_amount是20RUSD，所以请传入20RUSD及以上的funding_amount。_
   ...
   ```json
   {"jsonrpc":"2.0","result":{"temporary_channel_id":"0x942879c9ea8c20bacf8a0725399d137e918f1832b250081a8b670db332db3e46"},"id":2}
   ```
...
3. 查询nodeA和node1之间channels
   ...
   ```json
   {"jsonrpc":"2.0","result":{"channels":[{"channel_id":"0x08f61c5d8997b75abe130300c5a9319e4c3cdaf3b7504d06357340283377e56a","is_public":true,"channel_outpoint":"0x83058846872dec44d7a955d5e6ee4697dd1f871b24746a34d88c738037ecc77c00000000","peer_id":"QmXen3eUHhywmutEzydCsW4hXBoeVmdET2FJvMX69XJ1Eo","funding_udt_type_script":{"code_hash":"0x1142755a044bf2ee358cba9f2da187ce928c91cd4dc8692ded0337efa677d21a","hash_type":"type","args":"0x878fcc6f1f08d48e87bb1c3b3d5083f23f8a39c5d5c764f253b55b998526439b"},"state":{"state_name":"CHANNEL_READY","state_flags":[]},"local_balance":"0x77359400","offered_tlc_balance":"0x0","remote_balance":"0x0","received_tlc_balance":"0x0","latest_commitment_transaction_hash":"0xabd9de794d7a11f5eaad9039e847564c550b94a8d5113217587b4d840bb518d0","created_at":"0x1958e71c1da","enabled":true,"tlc_expiry_delta":"0x5265c00","tlc_fee_proportional_millionths":"0x3e8"}]},"id":3
   ```
...
4. 调用node2的`new_invoice`接口生成一个invoice
   ...
   amount设置成 0x5f5e100 (100000000)，也就是1个RUSD。
       ...
       "params": [
           {
               ...
               "expiry": "0xe10",
               "final_cltv": "0x28",
               "payment_preimage": "0xdd8f20cbdfc1be0db57a1886a25a62157d4ce56f64e91b1fbe1c39755a33755c",
               "hash_algorithm": "sha256",
               "udt_type_script": {
                   "code_hash": "0x1142755a044bf2ee358cba9f2da187ce928c91cd4dc8692ded0337efa677d21a",
                   "hash_type": "type",
...
5. 让nodeA付款前先查询一下各channel的local_balance和remote_balance
   ...
   从前面的步骤3可知`{"local_balance":"0x77359400","remote_balance":"0x0"}`
   ...
   ```
   找出response中`funding_udt_type_script`不为null的数据
   ```json
...
8. 再次查询各channel的local_balance和remote_balance
   ...
   从`{"local_balance":"0x77359400","remote_balance":"0x0"}`变为`{"local_balance":"0x654f5d20","remote_balance":"0x11e636e0"}`
   
   node1 ⟺ node2
   
   其余未变，`{"local_balance":"0x172a2c63bb","remote_balance":"0x1e4a8445"}`变为`{"local_balance":"0x17184ac0bb","remote_balance":"0x302c2745"}`
...
9. 关闭这个nodeA和node1的channel
   ...
   可以在ckb explorer上看到nodeA的地址新增了一笔+16.997RUSD的交易，也就是说在fiber节点上多次转账udt最终会在shutdown_channel关闭channel时在链上结算。
